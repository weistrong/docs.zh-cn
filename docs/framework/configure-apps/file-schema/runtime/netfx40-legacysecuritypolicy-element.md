...
> [!IMPORTANT]
> 应用 `<NetFx40_LegacySecurityPolicy>` 元素可能会导致本机映像生成器创建的本机映像程序集出现严重的性能下降 [ ( # A0) ](../../../tools/ngen-exe-native-image-generator.md) 未安装在 [全局程序集缓存](../../../app-domains/gac.md)中。 性能下降是由运行时在应用属性时无法将程序集加载为本机映像而导致其作为实时程序集加载的。
