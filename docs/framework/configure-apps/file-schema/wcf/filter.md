...
定义一个路由筛选器，该筛选器确定 <xref:System.ServiceModel.Dispatcher.MessageFilter> 要在评估传入消息时使用的 Windows Communication Foundation (WCF) ，以及筛选器所需的任何支持数据或参数。
...
| 元素 | 说明 |
| ------- | ----------- |
| [\<routing>](routing.md) | 用于定义一组路由筛选器的配置节，这些筛选器确定 <xref:System.ServiceModel.Dispatcher.MessageFilter> 计算传入消息时要使用 (WCF) Windows Communication Foundation 的类型。 |
