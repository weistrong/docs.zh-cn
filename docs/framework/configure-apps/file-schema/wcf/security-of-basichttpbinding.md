...
|-----------|-----------------|  
|无|-传输过程中消息不受保护。|  
|Transport|使用 HTTPS 传输提供安全性。 使用 HTTPS 对 SOAP 消息进行保护。 使用服务的 X.509 证书向客户端对服务进行身份验证。 使用所提供的 ClientCredentialType 对客户端进行身份验证。 请参阅 [\<transport>](transport-of-basichttpbinding.md) 。|  
