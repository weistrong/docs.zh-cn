...
```  
 ...
 若要创建包含架构的数据 **集** 的副本，并且仅创建表示 **已添加**、 **已修改** 或 **已删除** 行的数据，请使用 <xref:System.Data.DataSet.GetChanges%2A> **数据集** 的方法。 调用 **GetChanges** 时，还可以使用 **GetChanges** 来仅返回具有指定行状态的行 。 下面的代码示例演示如何在调用 **GetChanges** 时传递 **DataRowState** 。  
