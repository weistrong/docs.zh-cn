...
- 如果父代中不存在任何唯一约束，则每个表定义都有一个名为 *TableName* 的附加主键列 _Id 其中 *tablename* 是父表的名称。  
  
- 父表上的主键约束，通过将 **IsPrimaryKey** 属性设置为 **True**) ，将其他列标识为主键 (。 该约束以 Constraint\# 的形式来命名，其中 \# 为 1、2、3...。 例如，第一个约束的默认名称为 Constraint1。  
