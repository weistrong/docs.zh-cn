...
用数据填充 <xref:System.Data.DataSet> 对象后，您可以开始查询该对象。 使用 LINQ to DataSet 来表述查询类似于对其他启用 LINQ 的数据源使用 Language-Integrated 查询 (LINQ) 。 但请记住，在对对象使用 LINQ 查询时 <xref:System.Data.DataSet> ，您要查询对象的枚举， <xref:System.Data.DataRow> 而不是自定义类型的枚举。 这意味着你可以 <xref:System.Data.DataRow> 在 LINQ 查询中使用类的任意成员。 这使您可以创建丰富复杂的查询。  
