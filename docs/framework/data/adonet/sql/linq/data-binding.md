...
[!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 将 LINQ 查询转换为 SQL 以便在数据库上执行。 所得结果为强类型化的 `IEnumerable`。 由于这些对象是一般的公共语言运行库 (CLR) 对象，因此可以使用一般的对象数据绑定来显示结果。 另一方面，更改操作（插入、更新和删除）则需要额外的步骤。

## <a name="operation"></a>操作

隐式绑定到 Windows 窗体控件是通过实现 <xref:System.ComponentModel.IListSource> 完成的。 <xref:System.Data.Linq.Table%601> `Table<T>` 在 c # 中或 Visual Basic) 和泛型中的数据源泛型 (已 `Table(Of T)` `DataQuery` 更新为实现 <xref:System.ComponentModel.IListSource> 。 用户界面 (UI) 数据绑定引擎（Windows 窗体和 Windows Presentation Foundation）都会测试其数据源是否实现了 <xref:System.ComponentModel.IListSource>。 因此，如果将查询的直接影响结果写入控件的数据源，则会隐式调用 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 集合生成，如下例中所示：
...
- 数据源为 <xref:System.Linq.IQueryable%601>。 有两种情况：
  ...
  - 如果找 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 不到基础 <xref:System.Data.Linq.Table%601> ，则源不允许 edition (例如 `groupby`) 。 [!INCLUDE[vbtecdlinq](../../../../../../includes/vbtecdlinq-md.md)] 浏览查询以填充泛型 `SortableBindingList` ，这是一个简单的， <xref:System.ComponentModel.BindingList%601> 它实现了给定属性的 T 实体的排序功能。
...
如果使用的是 System.web，并将 EntitySet 绑定 \<TEntity> 到了 system.web.，则必须调用 entityset 的实体 \<TEntity> 。Entityset<tentity>.getnewbindinglist 更新 BindingSource. List。

如果使用的是 System.web 属性并设置了 BindingSource 属性，并将 BindingSource 设置为一个类，而该类在公开 EntitySet 的 DataMember 中具有名为的属性 \<TEntity> ，则无需调用 EntitySet \<TEntity> 。Entityset<tentity>.getnewbindinglist 更新 BindingSource，但丢失排序功能。
