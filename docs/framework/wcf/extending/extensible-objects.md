...
将扩展添加到集合时，将在 <xref:System.ServiceModel.IExtension%601.Attach%2A> 进入集合之前调用。 从集合中移除某个扩展 <xref:System.ServiceModel.IExtension%601.Detach%2A> 后，将在删除该扩展后调用。 这意味着 (假设适当的同步) 扩展只能在与之间的集合中找到 <xref:System.ServiceModel.IExtension%601.Attach%2A> <xref:System.ServiceModel.IExtension%601.Detach%2A> 。
...
如果集合中没有扩展名为，则 <xref:System.ServiceModel.IExtension%601> <xref:System.ServiceModel.IExtensionCollection%601.Find%2A> 返回 null，并 <xref:System.ServiceModel.IExtensionCollection%601.FindAll%2A> 返回一个空集合。 如果多个扩展实现 <xref:System.ServiceModel.IExtension%601> ，则 <xref:System.ServiceModel.IExtensionCollection%601.Find%2A> 返回其中一个。 从 <xref:System.ServiceModel.IExtensionCollection%601.FindAll%2A> 中返回的值是快照。
...
### <a name="extensible-objects-in-windows-communication-foundation"></a>Windows Communication Foundation 中的可扩展对象
 ...
 (WCF) Windows Communication Foundation 中有四个可扩展对象：
...
- <xref:System.ServiceModel.IContextChannel> –此接口允许检查由 WCF 运行时生成的通道和代理的每个状态。  此类的扩展名可以扩展 <xref:System.ServiceModel.IClientChannel> 的行为，也可以使用它存储每个通道的状态。
