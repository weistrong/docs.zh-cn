...
在运行 .NET Framework 4 的计算机上承载使用 .NET Framework 3.5 编写的 Windows Communication Foundation (WCF) 服务时，您可能会收到 <xref:System.ServiceModel.ProtocolException> 以下文本。
...
```  
 ...
 接下来，查找 <`compilers`> 元素，并将 CompilerVersion 提供程序选项更改为具有4.0 的值。 默认情况 `compiler` 下，<> 元素下有两个 <> 元素 `compilers` 。 您必须同时更新这两个元素的 CompilerVersion 提供程序选项，如下面的示例所示。  
...
1. 打开服务的 Web.config 文件，查找 <`compilation`> 元素。  
...
3. 查找 <`compilers`> 元素，并更改 CompilerVersion 提供程序选项的值为4.0。 默认情况 `compiler` 下，<> 元素下有两个 <> 元素 `compilers` 。 您必须同时更新这两个元素的 CompilerVersion 提供程序选项，如下面的示例所示。  
