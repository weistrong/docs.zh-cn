...
此示例演示如何使用 Windows Communication Foundation (WCF) 创建 ASP.NET 的异步 JavaScript 和 XML (AJAX) 服务，该服务可创建复杂类型的实例，并在服务和客户端之间以 JavaScript 对象表示法 (JSON) 的形式发送它们。 可以从 Web 浏览器客户端使用 JavaScript 代码来访问 AJAX 服务。 此示例以 [基本 AJAX 服务](basic-ajax-service.md) 示例为基础。
...
3. 定位到 `http://localhost/ServiceModelSamples/ComplexTypeClientPage.aspx` (不要在浏览器中从项目目录) 打开 complextypeclientpage.aspx。
