...
ms.locfileid: "99632160"
---
# <a name="basic-http-service"></a>基本 HTTP 服务

此示例演示如何使用 Windows Communication Foundation (WCF) REST 编程模型，实现一般称为 "POX" (纯旧 XML) service）的基于 HTTP 的 RPC 服务。 此示例由两个组件组成：一个自承载 WCF HTTP 服务 (Service.cs) 和一个控制台应用程序)  (用于创建服务并对其进行调用的控制台应用程序。

## <a name="sample-details"></a>示例详细信息

WCF 服务公开2个操作， `EchoWithGet` 并公开 `EchoWithPost` 作为输入传递的字符串。
...
App.config 文件使用 <xref:System.ServiceModel.Description.WebHttpEndpoint> 属性设置为 <xref:System.ServiceModel.Description.WebHttpEndpoint.HelpEnabled%2A> 的默认 `true` 来配置 WCF 服务。 因此，WCF 基础结构将在中创建一个基于 HTML 的自动帮助页 `http://localhost:8000/Customers/help` ，该帮助页提供有关如何构造对服务的 http 请求以及如何使用该服务的 HTTP 响应的信息。
