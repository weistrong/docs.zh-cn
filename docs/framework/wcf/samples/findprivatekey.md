...
使用证书进行安全的示例使用 *Setup.bat* 文件中的 FindPrivateKey 工具。 找到私钥文件后，可以使用其他工具（如 *Cacls.exe* ）将适当的访问权限设置到该文件。

当在用户帐户下运行 Windows Communication Foundation (WCF) 服务（如自承载可执行文件）时，请确保用户帐户对文件具有只读访问权限。 Internet Information Services () IIS 下运行 WCF 服务时，服务运行的默认帐户是 IIS 7 和更早版本上的网络服务，或者 IIS 7.5 及更高版本上的应用程序池标识。 有关详细信息，请参阅 [应用程序池标识](/iis/manage/configuring-security/application-pool-identities)。
...
出现这种情况时，请使用 FindPrivateKey 工具查找私钥文件，然后为运行服务的进程设置访问权限。 例如，可以通过 Cacls.exe 工具来完成此操作，如以下示例中所示：
...
若要下载此项目，请访问 [Windows Communication Foundation (WCF) ，并 Windows Workflow Foundation () 的 WF .NET Framework 示例](https://www.microsoft.com/download/details.aspx?id=21459)。
...
| `<thumbprint>`  | 证书的指纹 (你可以使用 Certmgr.exe 工具来查找此)  |
| `-f`            | 仅输出文件名                                                             |
| `-d`            | 仅输出目录                                                             |
| `-a`            | 输出绝对文件名                                                         |
