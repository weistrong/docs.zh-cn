...
> [!NOTE]
> 在执行本主题中的步骤之前，请运行该应用程序，启动每个类型的多个工作流，并针对每个工作流进行一个或两个猜测。 此步骤和下面的步骤中将使用这些持久工作流， [如何：更新正在运行的工作流实例的定义](how-to-update-the-definition-of-a-running-workflow-instance.md)。

### <a name="to-make-a-copy-of-the-numberguessworkflowactivities-project"></a><a name="BKMK_BackupCopy"></a> 创建 NumberGuessWorkflowActivities 项目的副本
...
6. 导航到 **NumberGuessWorkflowActivities\bin\debug** 文件夹 (或 **bin\release** ，具体取决于项目设置) 。 将 **NumberGuessWorkflowActivities.dll** 复制并粘贴到 **PreviousVersions** 文件夹中。

7. 将 **PreviousVersions** 文件夹中 **NumberGuessWorkflowActivities.dll** 重命名为 **NumberGuessWorkflowActivities_v1.dll**。

    > [!NOTE]
    > 本主题中的步骤演示了一种对用于包含多个版本的工作流的程序集进行管理的方法。 也可以使用其他方法，例如，对程序集进行强命名，并在全局程序集缓冲中注册这些程序集。

8. 在 **NumberGuessWorkflowHost**、 **NumberGuessWorkflowActivities** 和新添加 **PreviousVersions** 文件夹所在的文件夹中创建一个名为 **NumberGuessWorkflowActivities_du** 的新文件夹，并将 **NumberGuessWorkflowActivities** 文件夹中的所有文件和子文件夹复制到新的 **NumberGuessWorkflowActivities_du** 文件夹中。 此项目的备份副本将在 [如何：更新正在运行的工作流实例的定义](how-to-update-the-definition-of-a-running-workflow-instance.md)中使用。

9. 重新打开 Visual Studio 2012 中的 **WF45GettingStartedTutorial** 解决方案。

### <a name="to-update-the-workflows"></a><a name="BKMK_UpdateWorkflows"></a> 更新工作流

在本节中，将会更新工作流定义。 将会更新针对用户猜测提供反馈的两个 `WriteLine` 活动，并添加一个新的 `WriteLine` 活动，此活动提供猜测了数字之后有关该游戏的其他信息。

#### <a name="to-update-the-statemachine-workflow"></a><a name="BKMK_UpdateStateMachine"></a> 更新 StateMachine 工作流
...
#### <a name="to-update-the-flowchart-workflow"></a><a name="BKMK_UpdateFlowchart"></a> 更新流程图工作流
...
4. 将 " **WriteLine** " 活动从 "**工具箱**" 的 "**基元**" 部分拖放到 `True` 最顶部操作的下拉点上 `FlowDecision` 。 `WriteLine` 活动将添加到流程图并链接到 `True` 的 `FlowDecision` 操作。
...
#### <a name="to-update-the-sequential-workflow"></a><a name="BKMK_UpdateSequential"></a> 更新顺序工作流
...
4. 将 " **writeline** " 活动从 "**工具箱**" 的 "**基元**" 部分拖放到 " **DoWhile** " 活动后面，使 **WriteLine** 成为根活动中的最后一个活动 `Sequence` 。
...
### <a name="to-update-workflowversionmap-to-include-the-previous-workflow-versions"></a><a name="BKMK_UpdateWorkflowVersionMap"></a> 更新 WorkflowVersionMap 以包含以前的工作流版本
...
### <a name="to-build-and-run-the-application"></a><a name="BKMK_BuildAndRun"></a> 生成并运行应用程序
...
3. 打开 Windows 资源管理器并导航到 **NumberGuessWorkflowHost\bin\debug** 文件夹 (或 **bin\release** ，具体取决于你的项目设置) 并使用与已完成的工作流相对应的 Notepad 打开跟踪文件。 如果未记下 `InstanceId` ，可以使用 Windows 资源管理器中的 " **修改日期** " 信息来识别正确的跟踪文件。
...
4. 切换回数字猜测应用程序，然后选择在进行更新前启动的一个工作流。 您可通过查看在状态窗口下面显示的版本信息来识别当前选择的工作流的版本。 输入一些猜测，并注意状态更新与前一个版本的 `WriteLine` 活动输出相符，并且不包括用户猜测。 这是因为，这些工作流使用的是不具有 `WriteLine` 更新的前一个工作流定义。
    ...
    在下一步中， [如何：更新正在运行的工作流实例的定义](how-to-update-the-definition-of-a-running-workflow-instance.md)，将更新正在运行的 `v1` 工作流实例，使其包含作为实例的新功能 `v2` 。
