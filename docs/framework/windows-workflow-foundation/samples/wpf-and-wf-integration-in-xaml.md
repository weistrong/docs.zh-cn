---
description: 了解有关 XAML 中的 WPF 和 Windows Workflow Foundation 集成的详细信息
...
ms.locfileid: "99798103"
---
# <a name="wpf-and-windows-workflow-foundation-integration-in-xaml"></a>XAML 中的 WPF 和 Windows Workflow Foundation 集成

此示例演示如何创建一个应用程序，该应用程序使用 Windows Presentation Foundation (WPF) ，并 Windows Workflow Foundation 单个 XAML 文档中 (WF) 功能。 为实现此目的，此示例使用 Windows Workflow Foundation 和 XAML 扩展性。
...
## <a name="sample-details"></a>示例详细信息
 ...
 `ShowWindow`活动从 <xref:System.Activities.AsyncCodeActivity%601> 类派生，以显示 WPF 窗口，并在窗口关闭时完成。 `Window` 属性的类型为 `Func<Window>`，该类型允许在每次执行活动时按需创建窗口。 `Window` 属性使用一个 <xref:System.Xaml.XamlDeferringLoader> 来启用此延迟计算模型。 `FuncFactoryDeferringLoader` 允许在序列化期间捕获一个 `XamlReader`，然后在活动执行期间读取它。
