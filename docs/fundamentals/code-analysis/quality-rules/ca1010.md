...
您可以为这两个规则中的所有规则配置这些选项，也可以为此类别中的所有规则配置这些选项 ([设计](design-warnings.md)) 。 有关详细信息，请参阅 [代码质量规则配置选项](../code-quality-rule-options.md)。
...
- [将基类更改](#fix-by-base-class-change) 为已实现泛型和非泛型接口（如类）的类型 `Collection<T>` 。
