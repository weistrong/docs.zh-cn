...
当传递到反序列化方法或初始化为字段或属性的 [Newtonsoft.Json.JsonSerializer](https://www.newtonsoft.com/json/help/html/T_Newtonsoft_Json_JsonSerializer.htm)实例 *可能* 满足以下两个条件时，将触发此规则：
...
此规则类似于 [CA2329](ca2329.md)，但在这种情况下，分析无法确定是否将序列化程序配置为不安全地。
