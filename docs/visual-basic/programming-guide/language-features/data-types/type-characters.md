...
文本还可以使用标识符类型字符 (`%` 、 `&` 、 `@` 、 `!` 、 `#` 、) ，也可以 `$` 是变量、常量和表达式。 但 (、、、、、、) 文本类型字符只能 `S` `I` `L` `D` `F` `R` `C` 与文本一起使用。

在所有情况下，文本类型字符都必须紧跟在文本值之后。

## <a name="hexadecimal-binary-and-octal-literals"></a>十六进制、二进制和八进制文本

编译器通常将整数文本解释为十进制 (以10为底) 号系统。 你还可以将整数文本定义为带有前缀的十六进制 (以16为基数的) 号 `&H` ，作为二进制 (以前缀形式表示的以2为基的) 号 `&B` ，以及作为带有前缀的八进制 (基数 8) 号 `&O` 。 前缀后面的数字必须适用于数字系统。 下表对此进行了说明。  
  
|数字基数|前缀|有效的数字值|示例|
|-----------------|------------|------------------------|-------------|
|十六进制（以 16 为基数）|`&H`|0-9 和 A-f|`&HFFFF`|
|二进制 (基数 2) |`&B`|0-1|`&B01111100`|
